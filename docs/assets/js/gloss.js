...
/*! Gloss build: generated from uikit.js; do not edit. */
...
(function (global, factory) {
    ...
    const clsTransition = "gls-transition";
    ...
    const clsAnimation = "gls-animation";
        ...
        toNodes(element).map(
          (element2) => new Promise((resolve, reject) => {
            ...
            const classes = [
              ...
              origin && `gls-transform-origin-${origin}`,
    ...
    function getCoveringElement(target) {
      ...
      for (const position of top ? [0, top] : [0]) {
        ...
        for (const el of document.elementsFromPoint(left + width / 2, position)) {
          ...
          !hasClass(el, "gls-togglable-leave") && (hasPosition(el, "fixed") && zIndex(
    ...
    var Togglable = {
      ...
      data: {
        ...
        origin: false,
        transition: "ease",
        clsEnter: "gls-togglable-enter",
        clsLeave: "gls-togglable-leave"
      },
      computed: {
        ...
        updateNav() {
          ...
          for (const el of this.navItems) {
            ...
            if (isNumeric(cmd)) {
              ...
              toggleClass(button, "gls-disabled", !!this.parallax);
    ...
    var Slider = {
      ...
      data: () => ({
        ...
        clsActivated: "",
        clsEnter: "gls-slide-enter",
        clsLeave: "gls-slide-leave",
        clsSlideActive: "gls-slide-active",
        Transitioner: false,
        transitionOptions: {}
      }),
      connected() {
    ...
    var LightboxPanel = {
      ...
      data: () => ({
        ...
        clsFit: "gls-lightbox-items-fit",
        clsZoom: "gls-lightbox-zoom",
        attrItem: "gls-lightbox-item",
        selList: ".gls-lightbox-items",
        selClose: ".gls-close-large",
        selNav: ".gls-lightbox-thumbnav, .gls-lightbox-dotnav",
        selCaption: ".gls-lightbox-caption",
        selCounter: ".gls-lightbox-counter",
        pauseOnHover: false,
        velocity: 2,
        Animations,
        template: `<div class="gls-lightbox gls-overflow-hidden">  <div class="gls-lightbox-items"></div>  <div class="gls-position-top-right gls-position-small gls-transition-fade" gls-inverse>  <button class="gls-lightbox-close gls-close-large" type="button" gls-close></button>  </div>  <div class="gls-lightbox-slidenav gls-position-center-left gls-position-medium gls-transition-fade" gls-inverse>  <a href gls-slidenav-previous gls-lightbox-item="previous"></a>  </div>  <div class="gls-lightbox-slidenav gls-position-center-right gls-position-medium gls-transition-fade" gls-inverse>  <a href gls-slidenav-next gls-lightbox-item="next"></a>  </div>  <div class="gls-position-center-right gls-position-medium gls-transition-fade" gls-inverse style="max-height: 90vh; overflow: auto;">  <ul class="gls-lightbox-thumbnav gls-lightbox-thumbnav-vertical gls-thumbnav gls-thumbnav-vertical"></ul>  <ul class="gls-lightbox-dotnav gls-dotnav gls-dotnav-vertical"></ul>  </div>  <div class="gls-lightbox-counter gls-text-large gls-position-top-left gls-position-small gls-transition-fade" gls-inverse></div>  <div class="gls-lightbox-caption gls-position-bottom gls-text-center gls-transition-slide-bottom gls-transition-opaque"></div>  </div>`
      }),
      created() {
        ...
        remove$1($$(this.selNav, $el).filter((el) => !matches(el, `.gls-${navType}`)));
        for (const [i, item] of this.items.entries()) {
          append(list, "<div>");
          if (navType === "thumbnav") {
            wrapAll(
              toThumbnavItem(item, this.videoAutoplay),
              append($(this.selNav, $el), `<li gls-lightbox-item="${i}"><a href></a></li>`)
            );
          }
        }
        if (!this.slidenav) {
          remove$1($$(".gls-lightbox-slidenav", $el));
        }
        if (!this.counter) {
          remove$1($(this.selCounter, $el));
        }
        addClass(list, this.clsFit);
        const close = $("[gls-close]", $el);
      ...
      events: [
        ...
        },
        {
          ...
          async handler(_, item) {
            ...
            this.setItem(item, "<span gls-spinner gls-inverse></span>");
            if (!src) {
              return;
            }
            let matches2;
            const iframeAttrs = {
              ...
              "gls-responsive": "",
              "gls-video": Boolean(this.videoAutoplay)
            };
      ...
      methods: {
        ...
        setError(item) {
          this.setItem(item, '<span gls-icon="icon: bolt; ratio: 2" gls-inverse></span>');
        },
        showControls() {
    ...
    function toThumbnavItem(item, videoAutoplay) {
      const el = item.poster || item.thumb && (item.type === "image" || isImage(item.thumb)) ? createEl("img", { src: item.poster || item.thumb, alt: "" }) : item.thumb && (item.type === "video" || isVideo(item.thumb)) ? createEl("video", {
        ...
        playsinline: "",
        muted: "",
        "gls-video": videoAutoplay === "inline"
      }) : createEl("canvas");
      if (item.thumbRatio) {
    ...
    const selDisabled$1 = ".gls-disabled *, .gls-disabled, [disabled]";
    var lightbox = {
      install: install$3,
      props: { toggle: String },
      data: { toggle: "a" },
    ...
    function install$3(Gloss, Lightbox) {
    ...
    var notification = {
      ...
      created() {
        ...
        this.$mount(
          append(
            container,
            `<div class="${this.clsMsg}${this.status ? ` ${this.clsMsg}-${this.status}` : ""}" role="alert">  <a href class="${this.clsClose}" data-gls-close></a>  <div>${this.message}</div>  </div>`
          )
        );
      },
      async connected() {
    ...
    function toMedia(value, element) {
      if (isString(value)) {
        if (startsWith(value, "@")) {
          value = toFloat(css(element, `--gls-breakpoint-${value.slice(1)}`));
        } else if (isNaN(value)) {
          return value;
        }
      }
    ...
    var Position = {
      ...
      methods: {
        ...
        getPositionOffset(element = this.$el) {
          return toPx(
            this.offset === false ? css(element, "--gls-position-offset") : this.offset,
        ...
        getShiftOffset(element = this.$el) {
          return this.align === "center" ? 0 : toPx(
            css(element, "--gls-position-shift-offset"),
            this.axis === "y" ? "width" : "height",
            element
          ) * (includes(["left", "top"], this.align) ? 1 : -1);
        },
        getViewportOffset(element) {
          return toPx(css(element, "--gls-position-viewport-offset"));
        }
      }
    };
    ...
    var tooltip = {
      ...
      methods: {
        ...
        async _show(title, id) {
          this.tooltip = append(
            this.container,
            `<div id="${id}" class="gls-${this.$options.name}" role="tooltip">  <div class="gls-${this.$options.name}-inner">${title}</div>  </div>`
          );
    ...
    var drop = {
      ...
      methods: {
        ...
        isDelaying() {
          return [this.$el, ...$$(".gls-drop", this.$el)].some((el) => this.tracker.movesTo(el));
        },
        position() {
          const restoreScrollPosition = storeScrollPosition(this.$el);
          removeClass(this.$el, "gls-drop-stack");
          css(this.$el, this._style);
          this.$el.hidden = true;
          ...
          if (this.$el.offsetWidth > maxWidth) {
            addClass(this.$el, "gls-drop-stack");
          }
    ...
    var Dropnav = {
      ...
      data: {
        ...
        clsDrop: "gls-dropdown",
        clsDropbar: "gls-dropnav-dropbar",
        boundary: true,
        dropbar: false,
        dropbarAnchor: false,
        flip: true,
    ...
    var accordionIcon = "<svg width=\"13\" height=\"13\" viewBox=\"0 0 13 13\"><style>.gls-accordion-icon svg&gt;[class*=&quot;line-&quot;]{transition:0.2s ease-out;transition-property:transform, opacity;transform-origin:center}[aria-expanded=&quot;true&quot;] .gls-accordion-icon svg&gt;.line-1{transform:rotate(-45deg);opacity:0}[aria-expanded=&quot;true&quot;] .gls-accordion-icon svg&gt;.line-2{transform:rotate(90deg)}</style><rect width=\"13\" height=\"1\" fill=\"#000\" x=\"0\" y=\"6\" class=\"line-1\"/><rect width=\"1\" height=\"13\" fill=\"#000\" x=\"6\" y=\"0\" class=\"line-2\"/></svg>";
    ...
    var navbarToggleIcon = "<svg width=\"20\" height=\"20\" viewBox=\"0 0 20 20\"><style>.gls-navbar-toggle-icon svg&gt;[class*=&quot;line-&quot;]{transition:0.2s ease-in-out;transition-property:transform, opacity;transform-origin:center;opacity:1}.gls-navbar-toggle-icon svg&gt;.line-3{opacity:0}[aria-expanded=&quot;true&quot;].gls-navbar-toggle-icon svg&gt;.line-3,[aria-expanded=&quot;true&quot;]&gt;.gls-navbar-toggle-icon svg&gt;.line-3{opacity:1}[aria-expanded=&quot;true&quot;].gls-navbar-toggle-icon svg&gt;.line-2,[aria-expanded=&quot;true&quot;]&gt;.gls-navbar-toggle-icon svg&gt;.line-2{transform:rotate(45deg)}[aria-expanded=&quot;true&quot;].gls-navbar-toggle-icon svg&gt;.line-3,[aria-expanded=&quot;true&quot;]&gt;.gls-navbar-toggle-icon svg&gt;.line-3{transform:rotate(-45deg)}[aria-expanded=&quot;true&quot;].gls-navbar-toggle-icon svg&gt;.line-1,[aria-expanded=&quot;true&quot;]&gt;.gls-navbar-toggle-icon svg&gt;.line-1,[aria-expanded=&quot;true&quot;].gls-navbar-toggle-icon svg&gt;.line-4,[aria-expanded=&quot;true&quot;]&gt;.gls-navbar-toggle-icon svg&gt;.line-4{opacity:0}[aria-expanded=&quot;true&quot;].gls-navbar-toggle-icon svg&gt;.line-1,[aria-expanded=&quot;true&quot;]&gt;.gls-navbar-toggle-icon svg&gt;.line-1{transform:translateY(6px) scaleX(0)}[aria-expanded=&quot;true&quot;].gls-navbar-toggle-icon svg&gt;.line-4,[aria-expanded=&quot;true&quot;]&gt;.gls-navbar-toggle-icon svg&gt;.line-4{transform:translateY(-6px) scaleX(0)}</style><rect width=\"20\" height=\"2\" y=\"3\" class=\"line-1\"/><rect width=\"20\" height=\"2\" y=\"9\" class=\"line-2\"/><rect width=\"20\" height=\"2\" y=\"9\" class=\"line-3\"/><rect width=\"20\" height=\"2\" y=\"15\" class=\"line-4\"/></svg>";
    ...
    const NavParentIcon = {
      ...
      beforeConnect() {
        ...
        this.icon = this.$el.closest(".gls-nav-primary") ? `${icon}-large` : icon;
      }
    };
    const Search = {
      extends: IconComponent,
      ...
      beforeConnect() {
        const isToggle = hasClass(this.$el, "gls-search-toggle") || hasClass(this.$el, "gls-navbar-toggle");
        this.icon = isToggle ? "search-toggle-icon" : hasClass(this.$el, "gls-search-icon") && this.$el.closest(".gls-search-large") ? "search-large" : this.$el.closest(".gls-search-medium") ? "search-medium" : this.$props.icon;
      ...
      update: {
        read() {
          ...
          for (const target of toNodes(this.target)) {
            ...
            if (color !== false) {
              replaceClass(target, "gls-light gls-dark", color);
            }
          }
        }
      }
    ...
    function findTargetColor(target) {
      ...
      for (const percent of [0.25, 0.5, 0.75]) {
        ...
        for (const element of elements) {
          ...
          const color = css(element, "--gls-inverse");
          if (color) {
            if (color === last) {
              return `gls-${color}`;
            }
    ...
    var leader = {
      ...
      computed: {
        fill: ({ fill }, $el) => fill || css($el, "--gls-leader-fill-content")
      },
      connected() {
    ...
    var modal = {
      ...
      data: {
        ...
        selClose: '[class*="gls-modal-close"]'
      },
      events: [
        {
    ...
    function install({ modal }) {
      modal.dialog = function(content, options) {
        const dialog = modal($(`<div><div class="gls-modal-dialog">${content}</div></div>`), {
          stack: true,
          role: "alertdialog",
          ...options
        });
      ...
      modal.alert = function(message, options) {
        return openDialog(
          ({ i18n }) => `<div class="gls-modal-body">${isString(message) ? message : html(message)}</div>  <div class="gls-modal-footer gls-text-right">  <button class="gls-button gls-button-primary gls-modal-close" type="button" autofocus>${i18n.ok}</button>  </div>`,
          options
        );
      };
      modal.confirm = function(message, options) {
        return openDialog(
          ({ i18n }) => `<form>  <div class="gls-modal-body">${isString(message) ? message : html(message)}</div>  <div class="gls-modal-footer gls-text-right">  <button class="gls-button gls-button-default gls-modal-close" type="button">${i18n.cancel}</button>  <button class="gls-button gls-button-primary" autofocus>${i18n.ok}</button>  </div>  </form>`,
          options,
          () => Promise.reject()
        );
      };
      modal.prompt = function(message, value, options) {
        const promise = openDialog(
          ({ i18n }) => `<form class="gls-form-stacked">  <div class="gls-modal-body">  <label>${isString(message) ? message : html(message)}</label>  <input class="gls-input" autofocus>  </div>  <div class="gls-modal-footer gls-text-right">  <button class="gls-button gls-button-default gls-modal-close" type="button">${i18n.cancel}</button>  <button class="gls-button gls-button-primary">${i18n.ok}</button>  </div>  </form>`,
          options,
          () => null,
          () => input.value
        );
    ...
    const clsNavbarTransparent = "gls-navbar-transparent";
    var navbar = {
      extends: Dropnav,
      props: {
      ...
      data: {
        ...
        selNavItem: ".gls-navbar-nav > li > a,a.gls-navbar-item,button.gls-navbar-item,.gls-navbar-item a,.gls-navbar-item button,.gls-navbar-toggle",
        // Simplify with :where() selector once browser target is Safari 14+
        dropbarTransparentMode: false
      },
      computed: {
        navbarContainer: (_, $el) => $el.closest(".gls-navbar-container")
      },
      watch: {
        items() {
          const justify = hasClass(this.$el, "gls-navbar-justify");
          const containers = $$(".gls-navbar-nav, .gls-navbar-left, .gls-navbar-right", this.$el);
          for (const container of containers) {
            const items = justify ? $$(".gls-navbar-nav > li > a, .gls-navbar-item, .gls-navbar-toggle", container).length : "";
      ...
      methods: {
        getTransparentMode(el) {
          ...
          if (drop && hasClass(el, "gls-dropbar")) {
        ...
        write({ overflow }) {
          for (let i = 0; i < 2; i++) {
            ...
            if (!overflow[i - 1]) {
              ...
              css(this.$el, {
                "--gls-overflow-fade-start-opacity": toValue(percent),
                "--gls-overflow-fade-end-opacity": toValue(1 - percent)
              });
            }
          }
        },
    ...
    const clsInView = "gls-scrollspy-inview";
    var scrollspy = {
      args: "cls",
      props: {
        cls: String,
      ...
      methods: {
        toggle(el, inview) {
          ...
          let match;
          if (match = state.cls.match(/\bgls-animation-[\w-]+/g)) {
      ...
      update: [
        {
          read({ height: height$1, width, margin, sticky }, types) {
            ...
            const topOffset = offset(referenceElement).top - // offset possible `transform: translateY` animation 'gls-animation-slide-top' while hiding
          ...
          write(data, types) {
            ...
            if (this.inactive || scroll2 < start || this.showOnUp && (scroll2 <= start || dir === "down" && isScrollUpdate || dir === "up" && !this.isFixed && !below)) {
              ...
              if (this.animation && below) {
                if (hasClass(this.$el, "gls-animation-leave")) {
                  return;
                }
    ...
    const clsTransitionDisable = "gls-transition-disable";
    ...
    function applyAttributes(el) {
      const { $el } = this;
      addClass(el, attr($el, "class"), "gls-svg");
    ...
    function applyAnimation(el) {
      const length = getMaxPathLength(el);
      if (length) {
        css(el, "--gls-animation-stroke", length);
      }
    }

    const selDisabled = ".gls-disabled *, .gls-disabled, [disabled]";
    var Switcher = {
      mixins: [Togglable],
      args: "connect",
      props: {
    ...
    var tab = {
      ...
      data: {
        ...
        selVertical: ".gls-tab-left,.gls-tab-right"
      },
      connected() {
