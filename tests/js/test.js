...
(function (factory) {
    typeof define === 'function' && define.amd ? define('glosstest', factory) : factory();
})(function () {
    'use strict';
    ...
    function toNodes(element) {
        return (
            (element && (isNode(element) ? [element] : Array.from(element).filter(isNode))) || []
        );
    }
            ...
            () =>
                requestAnimationFrame(() => {
                    ...
                    const $container = prepend(
                        $body,
                        ` <div class="gls-container"> <select class="gls-select gls-form-width-small" style="margin: 20px 20px 20px 0"> <option value="index.html">Overview</option> ${tests
                            .map(
                                (name) =>
                                    `<option value="${name}.html">${name.split('-').map(ucfirst).join(' ')}</option>`,
                            )
                            .join(
                                '',
                            )} </select> <select class="gls-select gls-form-width-small" style="margin: 20px"> ${Object.keys(
                            styles,
                        )
                            .map((style2) => `<option value="${style2}">${styles[style2]}</option>`)
                            .join(
                                '',
                            )} </select> <select class="gls-select gls-form-width-small" style="margin: 20px"> ${Object.keys(
                            variations,
                            ...
                            .join(
                                '',
                            )} </select> <!--<label style="margin: 20px"> <input type="checkbox" class="gls-checkbox"/> <span style="margin: 5px">RTL</span> </label>--> </div> `,
                    );
