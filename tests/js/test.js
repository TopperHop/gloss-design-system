/*! Gloss 2.8.37 | https://utahhealthcare.github.io/gloss-docs/ | (c) 2019 - 2026 University of Utah Health | BSD-3-Clause License */

(function (factory) {
    typeof define === 'function' && define.amd ? define('glosstest', factory) :
    factory();
})((function () { 'use strict';
    ...
    function boxModelAdjust(element, prop, sizing = "border-box") {
      return css(element, "boxSizing") === sizing ? dirs[prop].map(ucfirst).reduce(
    ...
    const tests = ["accordion", "alert", "align", "animation", "arrow", "article", "aspect-ratio", "autocomplete", "background", "badge", "base", "border", "breadcrumb", "button", "card", "close", "column", "comment", "container", "countdown", "cover", "description-list", "divider", "dotnav", "drop", "dropbar", "dropdown", "filter", "flex", "form", "grid-masonry", "grid-parallax", "grid", "heading", "height-expand", "height-viewport", "height", "icon", "iconnav", "image", "label", "leader", "lightbox", "line", "link", "list", "margin", "marker", "modal", "nav", "navbar", "notification", "offcanvas", "overlay", "padding", "pagination", "parallax", "placeholder", "position", "progress", "rating", "scroll", "scrollspy", "search", "section", "slidenav", "slider", "slideshow", "social", "sortable", "spinner", "stepper", "sticky-navbar", "sticky-parallax", "sticky", "subnav", "svg", "switcher", "tab", "table", "text", "thumbnav", "tile", "toggle", "tooltip", "totop", "transition", "upload", "utility", "video", "visibility", "width", "z-index"];
      ...
      () => setTimeout(
        () => requestAnimationFrame(() => {
          ...
          const $container = prepend(
            $body,
            ` <div class="gls-container"> <select class="gls-select gls-form-width-small" style="margin: 20px 20px 20px 0"> <option value="index.html">Overview</option> ${tests.map(
          (name) => `<option value="${name}.html">${name.split("-").map(ucfirst).join(" ")}</option>`
        ).join("")} </select> <select class="gls-select gls-form-width-small" style="margin: 20px"> ${Object.keys(styles).map((style2) => `<option value="${style2}">${styles[style2]}</option>`).join("")} </select> <select class="gls-select gls-form-width-small" style="margin: 20px"> ${Object.keys(variations).map((name) => `<option value="${name}">${variations[name]}</option>`).join("")} </select> <!--<label style="margin: 20px"> <input type="checkbox" class="gls-checkbox"/> <span style="margin: 5px">RTL</span> </label>--> </div> `
          );
